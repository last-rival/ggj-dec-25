...
func _get_source_folder() -> String:
	var cfg_folder = ProjectSettings.get_setting("stuff/game_stuff/dialogue/source_folder") if ProjectSettings.has_setting("dialogue/source_folder") else null
	var folder = dialogue_folder if dialogue_folder else cfg_folder
	# https://github.com/godotengine/godot/issues/56598
	return folder if folder else "res://stuff/game_stuff/dialogues/"
